...
/**
 * The frozen `RefineryExtract` JSON contract, v1: the single hand-off between this tool, the basetool
 * frontend upload and the backend import. A breaking change requires bumping [SCHEMA_VERSION]; the
 * backend accepts `schemaVersion == 1` only.
 */
@Serializable
...
/**
 * The `rowIndex`es whose `inputQuantity` is `null`, which the ingest edge rejects, so the export
 * screen blocks the send on them. Spans every order in on-screen order; empty when every row has a
 * quantity.
 */
...
/**
 * The indexes of orders without a source image, which the ingest edge rejects (REQ-INGEST-010);
 * empty when every order carries its provenance.
 */
fun RefineryExtract.ordersMissingSourceImages(): List<Int> =
    orders.indices.filter { orders[it].sourceImages.isEmpty() }

/**
 * The indexes of orders without a goods row, which the ingest edge rejects; empty when every order
